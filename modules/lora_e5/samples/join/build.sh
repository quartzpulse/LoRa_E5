#!/usr/bin/env bash
#
# build.sh -- build (and optionally flash) samples/join for a real
# ESP32-S3 board. lora_e5 isn't in the west manifest, so
# ZEPHYR_EXTRA_MODULES is required every time; this script exists so
# that doesn't have to be remembered/retyped.
#
# Usage:
#   ./build.sh                       # build only
...
#
# Build output goes to <repo_root>/build/join_esp32s3 (out-of-tree, not
# samples/join/build -- keeps the sample directory clean regardless of
...
BUILD_DIR="$REPO_ROOT/build/join_esp32s3"
...
export ZEPHYR_EXTRA_MODULES="$REPO_ROOT/modules/lora_e5"
